...
class BatchNorm(object):
    ...
    def backward(self, delta):
        ...
        m = delta.shape[0]
        dvar = - np.sum(dnorm * (x_hat/(2*(sqrt_Var**3))), axis=0)
        dmu = - np.sum(dnorm / sqrt_Var, axis=0) - (2 / m) * dvar * np.sum(x_hat, axis=0)
        dx = (dnorm/sqrt_Var) + (dvar * (2/m) * x_hat) + (dmu / m)
