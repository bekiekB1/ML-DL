...
class knnClassifier(): # Change to KNN_classifier, even at testing
    def __init__(self):
        pass
    ...
    def predict(self,X):
        ...
        for i in range(X.shape[0]):
            ...
            if(self.weights == 'uniform' and self.distance == 'Manhattan'):
                indx_opt = np.argpartition(np.linalg.norm(X[i]-self.X,axis=1) , self.k)  
            elif(self.weights == 'uniform' and self.distance != 'Manhattan'):
                indx_opt = np.argpartition(np.linalg.norm(X[i]-self.X,2,axis=1) , self.k) 
            elif(self.distance == 'Manhattan'):
                indx_opt = np.argpartition(1 / np.linalg.norm(X[i]-self.X,axis=1) , self.k)  
            else:
                indx_opt = np.argpartition(1 / np.linalg.norm(X[i]-self.X,2,axis=1) , self.k)                          
            indx_opt = indx_opt[:self.k]##waht if we have two points at min distance
